    ...
    return `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;">
        ${folders.map(f => {
            ...
            return `<div class="dubin-subfolder">
                <div class="dubin-subfolder-title" style="color:${f.color};">
                    <i class="fas ${f.icon}"></i> ${f.label}
                </div>
                <p style="font-size:11px;color:var(--text-dim);margin-bottom:10px;">${f.desc}</p>
                ${link ? `<a href="${esc(link)}" target="_blank" class="btn btn-sm" style="background:${f.color};text-decoration:none;display:inline-flex;align-items:center;gap:5px;">
                    <i class="fas fa-external-link-alt"></i> Открыть в Drive
                </a>` : `<span style="font-size:11px;color:var(--text-dim);">Папка не настроена</span>`}
            </div>`;
        }).join('')}
    </div>`;
}
...
function renderVoiceFiles(files, projId, isAdmin) {
    return `<div style="margin-top:12px;">
        <p style="font-size:11px;font-weight:800;color:var(--dub);text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">
            <i class="fas fa-microphone-alt"></i> Загруженная озвучка
        </p>
        ${files.map((f, i) => `<div class="dubin-file-row">
            ...
            <div style="display:flex;gap:6px;flex-shrink:0;">
                <a href="${esc(f.link)}" target="_blank" class="btn btn-sm btn-purple" style="text-decoration:none;">
                    <i class="fas fa-download"></i>
                </a>
                ${isAdmin ? `<button class="btn-sm" style="background:#ef4444;" onclick="deleteVoiceFile('${projId}',${i})">
...
async function renderProjects(db, isAdmin) {
    ...
    document.getElementById('dubin-projects-list').innerHTML = projects.length === 0
        ? `<div style="text-align:center;padding:30px;color:var(--text-dim);">
            <i class="fas fa-folder-open" style="font-size:2.5rem;margin-bottom:12px;display:block;opacity:0.3;"></i>
            ...
            <div class="dubin-folder">
                ...
                <div class="dubin-folder-body" id="folder-${p.id}">
                    ...
                    <div style="margin-top:14px;padding-top:14px;border-top:1px solid var(--border);">
                        <button class="btn btn-purple btn-sm" onclick="openUploadVoice('${p.id}')">
                            <i class="fas fa-upload"></i> Загрузить озвучку
                        </button>
                        ${p.voiceFiles && p.voiceFiles.length > 0 
                            ? renderVoiceFiles(p.voiceFiles, p.id, isAdmin) 
                            : '<p style="font-size:12px;color:var(--text-dim);margin-top:10px;">Озвучка ещё не загружена</p>'}
                    </div>
                </div>
            </div>
        </div>`).join('');
...
export function bindDubin(db, auth, getState) {
    ...
    window.saveDubinProject = async () => {
        ...
        if (!id) {
            ...
            await addDoc(collection(db, 'dubinProjects'), data);
        } else {
            await updateDoc(doc(db, 'dubinProjects', id), data);
        }
        ...
        try {
            if (window.emailjs) {
                await emailjs.send(
                    ...
                    EMAILJS_CONFIG.templateSuggest,
                    {
                        to_email: 'nekosoundstudio@gmail.com',
                        from_name: senderName,
                        from_email: senderEmail || 'нет',
                        title,
                        media_type: type,
                        link: link || 'не указана',
                        reason,
